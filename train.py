...
def train(epochs=50, device="cpu", pretrained=None):
    ...
    dataset = ObjectDetectionDataset(
        r"D:\deep_learning\object_detection\data\images\train", img_size=img_size
    )
    loader = DataLoader(
        dataset,
        batch_size=32,
    ...
    for epoch in range(epochs):
        ...
        scheduler.step()
        n = len(loader)
        avg_cls = running["cls"] / n
        avg_box = running["box"] / n
        avg_dfl = running["dfl"] / n
        current_lr = scheduler.get_last_lr()[0]

        print(
            f"epoch {epoch:3d} | lr {current_lr:.5f} | "
            f"cls {avg_cls:.3f} box {avg_box:.3f} dfl {avg_dfl:.3f}"
        )

        with open("./log/log.txt", "a", encoding="UTF-8") as f:
            f.write(
                f"Epoch {epoch}: cls={avg_cls:.4f}, box={avg_box:.4f}, "
                f"dfl={avg_dfl:.4f}, lr={current_lr:.6f}\n"
            )

        torch.save(model.state_dict(), "./ckpt/checkpoint_last.pth")
...
if __name__ == "__main__":
    ...
    train(
        epochs=50,
        device=device,
        pretrained=rf"./pretrained_model/pretrained_cnn_23.pth",
    )
