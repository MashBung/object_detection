...
class DetectionLoss(nn.Module):
    ...
    def forward(self, preds, gt_labels, gt_bboxes, mask_gt):
        ...
        if fg_mask.any():  # fg_mask (32,2100) bool, N = positive 앵커 수
            ...
            pred_dist_pos = pred_dist[fg_mask].view(-1, 4, self.reg_max)  # (N, 4, 16)
            target_ltrb_pos = target_ltrb[fg_mask]  # (N, 4)
            loss_dfl = (
    ...
    def df_loss(self, pred_dist, target_dist):
        # pred_dist:   (N, 4, 16)  positive의 분포 로짓
        # target_dist: (N, 4)           정답 거리 (격자 단위, 0~reg_max-1)
        tl = target_dist.long()  # (N, 4)
        tr = tl + 1  # (N,4)
        ...
        return loss.mean(-1, keepdim=True)  # 4개 변 평균 -> (N, 1)
